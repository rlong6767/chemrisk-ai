...
This project is intentionally framed as **decision support**, not automated safety approval. It shows how machine learning can help organize conservative screening data, surface review cases, and convert pair-level chemical incompatibility information into direction-aware tank/service recommendations. In this context, **direction-aware** means the workflow separately evaluates scenarios such as `A_into_B` and `B_into_A`, because the stored chemical, incoming contaminant, inventory, dilution, mixing, and tank context can change whether an RTD is useful or creditable.
...
Chemical manufacturing and bulk-liquid storage operations commonly rely on chemical compatibility information, such as compatibility charts, SDS guidance, segregation rules, process knowledge, and site-specific review tables. These tools may identify whether two materials are compatible or incompatible, but they often do not directly answer what safeguard is most appropriate if the materials are accidentally mixed.

More detailed incompatibility screens may identify potential consequences such as heat generation, gas evolution, toxic vapor release, pressure rise, fire, corrosion, or fouling.
...
- acidic/sulfonic surfactant plus alkanolamine clean liquid-phase exotherm patterns,
- reviewed alkali-base plus chlorinated-oxidizer heat-of-dilution / no-significant-reaction patterns,
...
### 3. Targeted chemistry-pathway calibration updates

The latest notebook adds two targeted mechanism-pathway updates using the same sanitized generic labels/classes as the public dataset:

- **LF17 - acidic/sulfonic surfactant + alkanolamine clean exotherm:** preserves a positive RTD-candidate pathway when the screened chemistry is consistent with a liquid-phase acid/base exotherm that may create a measurable bulk temperature rise.
- **LF18 - alkali-base + chlorinated-oxidizer heat-of-dilution / no-significant-reaction pattern:** adds a calibrated no-RTD pathway where reviewed chemistry indicates the scenario is better described by heat of dilution under the modeled assumptions rather than by an RTD-creditable incompatible reaction.

These rules are intentionally expressed with generic chemical classes rather than private site chemical names. The public notebook documents the generic mechanism logic; a private site map can then connect those generic classes to actual tank services, SDS reviews, and project-specific evidence.

### 4. Leakage-safe grouped modeling
...
### 5. Model-assisted review overlay

The final model does **not** silently override the mechanism-aware weak-label layer. If the model disagrees with the calibrated labeling logic, the scenario is routed to **Review**. Model-positive / weak-label-negative disagreements are especially important because they may reveal a missing positive chemistry pathway rather than a true no-RTD case.

This is intentional. In a safety-critical workflow, model disagreement is a signal for engineering attention, not a reason to automate a safeguard decision. The current public notebook includes added LF17 and LF18 pathways: LF17 keeps acidic/sulfonic surfactant plus alkanolamine clean liquid-phase exotherms in RTD-candidate review, while LF18 gives reviewed alkali-base plus chlorinated-oxidizer heat-of-dilution cases a specific no-RTD chemistry basis instead of relying on generic gas/pressure/fouling text.

### 6. Explanation layers: row-level, directional, and service-level design basis
...
The explanation output also separates **strong positive chemistry basis** cases from **weaker screening-retained** cases. Stronger cases have a specific bulk-thermal chemistry basis, such as bulk acid/base exotherm, protected acid/base RTD pathway, acidic/sulfonic surfactant plus alkanolamine clean exotherm pathway, or calibrated RTD-positive pattern. Weaker screening-retained cases have high source severity plus heat and were not eliminated by the no-RTD failure-mode screen, but they require stronger engineering review before final RTD credit.

For non-candidate scenarios, the explanation layer now also includes targeted no-RTD reasons such as the LF18 alkali-base / chlorinated-oxidizer heat-of-dilution pattern, so a reviewer can distinguish a generic gas/pressure/fouling rejection from a more specific reviewed chemistry basis.
...
### 7. Direction-aware tank/service layer
...
### 8. Audit-safe summary and long-form scenario exports

The notebook separates management-level summary output from the detailed audit trail. The one-row-per-service or one-row-per-tank summary is kept Excel/CSV-safe by omitting or truncating very long scenario-detail fields. Complete pair/direction reasoning is preserved in long-form audit exports, including RTD-candidate details, non-candidate details, and all-scenario detail tables. This avoids relying on giant wrapped spreadsheet cells when an engineer needs to trace why a specific pair was retained, rejected, or routed to review.
...
**Note:** If the notebook is rerun after the LF17/LF18 targeted chemistry-pathway updates, the model-card, summary outputs, and figures should be regenerated so the published metrics and counts remain synchronized with the latest code.
...
| Review - model challenges weak label / RTD-candidate review needed | 22 |

The latest public notebook adds an explicit LF17 chemistry pathway for acidic/sulfonic surfactant plus alkanolamine clean liquid-phase exotherms. If the notebook is re-run with refreshed inputs, regenerate the summary tables, model card, and figures so the published counts remain synchronized with the code.
...
- a generic chemical vocabulary and mapping guide for understanding sanitized labels/classes,
- a design document,
- model-card and summary outputs,
- and final figures.

The `data/` folder includes schema-only CSVs plus a generic chemical vocabulary and mapping guide. Matching the column headers alone is not enough to make a new site dataset meaningful. Actual chemicals must be mapped into the generic ChemRisk-AI labels/classes using SDS information, compatibility-matrix context, concentration/grade checks, and SME review.

To run the notebook end-to-end, provide private or synthetic input files that match the schemas, generalized chemical vocabulary, and categorical value conventions documented in `data/`. A fully runnable synthetic demo is planned as future work.

When the notebook is run with the required inputs, it can also generate additional design-basis exports such as service-level explanations, RTD-candidate scenario details, enhanced chemistry-basis summaries, Excel-safe summary tables, and long-form audit tables. Those generated CSV files are useful for private project review, but they should be checked for site-specific or audit-sensitive details before being published.
...
- enhanced chemistry-basis explanations for retained RTD-candidate scenarios,
- and long-form audit detail exports for traceability.
...
1. Inspect `notebooks/01_chemrisk_ai_phase1_mvp_final_public_v11.ipynb`.
...
- The model is trained on weak labels and limited audit calibration, not a broad incident-outcome or laboratory reaction-test database. New lab or outcome evidence should be treated as calibration/validation evidence and incorporated through controlled version updates.
